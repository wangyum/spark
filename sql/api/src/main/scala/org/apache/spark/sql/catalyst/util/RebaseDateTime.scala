...
object RebaseDateTime {
  ...
  // `JsonRebaseRecord`. HashMap is used here instead of Scala's immutable map because
  ...
  private[sql] def loadRebaseRecords(fileName: String): HashMap[String, RebaseInfo] = {
    ...
    val hashMap = new HashMap[String, RebaseInfo](jsonRebaseRecords.size, HashMap.defaultLoadFactor)
  ...
  def rebaseJulianToGregorianMicros(timeZoneId: String, micros: Long): Long = {
    if (micros >= lastSwitchJulianTs) {
      ...
      val rebaseRecord = julianGregRebaseMap.getOrElse(timeZoneId, null)
      if (rebaseRecord == null || micros < rebaseRecord.switches(0)) {
