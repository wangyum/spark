...
class AdaptiveQueryExecSuite
  ...
  test("coalesce shuffle partitions under a left semi range join") {
    // A semi join emits at most one row per streamed row, so the advisory size applies
    // even when the minimum partition size is one byte.
    withSQLConf(
      SQLConf.RANGE_JOIN_ENABLED.key -> "true",
      SQLConf.ADVISORY_PARTITION_SIZE_IN_BYTES.key -> "64MB",
      ...
      val (_, adaptivePlan) = runAdaptiveAndVerifyResult(
        "SELECT * FROM (SELECT value v, max(key) k FROM testData GROUP BY value) t " +
          "LEFT SEMI JOIN (SELECT id AS a FROM range(1)) u ON t.k > u.a")
      val planned = collect(adaptivePlan) {
        case j: BroadcastRangeJoinExec => j
      }
      assert(planned.size == 1, adaptivePlan)
      assert(planned.head.joinType.sql == "LEFT SEMI")
      val coalescedReads = collect(adaptivePlan) {
        case read: AQEShuffleReadExec if read.isCoalescedRead => read
      }
      assert(coalescedReads.nonEmpty, adaptivePlan)
    }
  }
