...
//      Row("b", 8) :: Row("c", 10) :: Nil)
 ...
 //   sql("select month, area from windowData qualify month > 2").show

//    checkAnswer(
//      sql(
//        """
//          |select month, area, max(product) over (partition by area) as m
//          |from windowData qualify m > 2
//          """.stripMargin),
...
//          |from windowData
//          |qualify w > 100 AND sum(product) > 5
//          """.stripMargin),
...
//        """
//          | select w.area
//          | from windowData w JOIN windowData w2 on w.area=w2.area
//          | qualify row_number() over (partition by w.area order by w.product) IN (2)
//        """.stripMargin),
//      Seq(
//        ("a"),
//        ("b"),
...
//          |qualify w > 1 and sum(product) > 5
//          """.stripMargin),
//      Row("a", 1) :: Row("b", 2) ::
//        Row("b", 2) :: Row("c", 2) :: Row("c", 2) :: Nil)

    sql("select * from windowData Qualify area < 10 and count(month) > 2").explain(true)
