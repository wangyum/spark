...
import org.apache.spark.sql.execution.joins.{BroadcastHashJoinExec, BroadcastNestedLoopJoinExec,
  BroadcastRangeJoinExec, CartesianProductExec}
...
case class CoalesceShufflePartitions(session: SparkSession) extends AQEShuffleReadRule {
  ...
  private def isExplodingJoin(p: SparkPlan): Boolean = p match {
    ...
    // Semi and anti emit at most one row per streamed row. Inner and outer can multiply.
    case j: BroadcastRangeJoinExec => j.joinType match {
      case LeftSemi | LeftAnti => false
      case _ => true
    }
