...
private[execution] object RangeIndex {
  ...
  /**
   * Type the index compares. A UDT is stored as its `sqlType`, and that may itself
   * be a UDT, so unwrap until the physical type. Same rule as `InternalRow.getAccessor`.
   */
  def indexedType(dt: DataType): DataType = dt match {
    case udt: UserDefinedType[_] => indexedType(udt.sqlType)
    case other => other
  }
...
private[execution] object IntervalIndex {
  ...
  private def buildFromSorted(
    ...
    events.foreach { event =>
      ...
      // Rows newly active at this key start here. Rows before it span below.
...
private[execution] class IntervalIndex(
  ...
  private def lowerBound(value: Any): Int = {
    ...
    while (lo < hi) {
      val mid = lo + ((hi - lo) >>> 1)
      if (keyCmp(keys(mid), value) < 0) lo = mid + 1
      else hi = mid
    }
    lo
  }
   ...
   *  - `first < last`: active rows at `first`, then rows activated on `(first, last]`.
   *  - `first == last`: only those active rows. The probe did not land on a key.
   *  - `first > last`: `low > high` and a key lies strictly between them. Only rows
   *    already active before `keys(first)` can match; `activeNewOffsets(first)` is
   *    where rows that start at that key begin.
   */
