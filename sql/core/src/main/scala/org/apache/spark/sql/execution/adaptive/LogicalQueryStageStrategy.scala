...
import org.apache.spark.sql.catalyst.planning.{
  ExtractEquiJoinKeys, ExtractRangeJoinKeys, ExtractSingleColumnNullAwareAntiJoin, RangeJoin}
...
import org.apache.spark.sql.execution.joins.{
  ...
  HashedRelationBroadcastMode, RangeBroadcastMode}
...
object LogicalQueryStageStrategy extends Strategy {
  ...
  private object RangeBroadcastJoinStage {
    def unapply(plan: LogicalPlan): Option[RangeBroadcastJoinStage] = plan match {
      case j @ Join(_, _, joinType, Some(cond), _) =>
        j match {
          case ExtractRangeJoinKeys(left, right, leftKeys, rightKeys, _, rangeJoin) =>
            def fromMode(
              ...
              if (!mode.indexKind.matches(rangeJoin) ||
                ...
                // serve a join on column z. The mode stores keys bound at planning
                // time. Nullability can change when AQE rewrites the stage, so it
                // is not part of the comparison.
                val bound = BindReferences.bindReferences(buildKeys, buildPlan.output)
                if (sameBoundKeys(mode.buildKeys, bound)) {
    ...
    private def sameBoundKeys(modeKeys: Seq[Expression], bound: Seq[Expression]): Boolean = {
      def relax(expr: Expression): Expression = expr.transform {
        case b: BoundReference => b.copy(nullable = true)
      }
      modeKeys.corresponds(bound)((left, right) => relax(left).semanticEquals(relax(right)))
    }
  }
