        ...
        def createRangeJoin(hintOnly: Boolean) = {
          def createBroadcastRangeJoinExec(
            ...
            joins.BroadcastRangeJoinExec(
              leftRangeKeys, rightRangeKeys, joinType, buildSide, j.condition, rangeJoin,
              planLater(left), planLater(right))
          }

          if (!conf.rangeJoinEnabled) {
            None
          } else {
            plan match {
              case ExtractRangeJoinKeys(_, _, leftKeys, rightKeys, _, rangeJoin) =>
                getBroadcastBuildSide(j, hintOnly, conf).collect {
                  case buildSide
                      if joins.BroadcastRangeJoinExec.supports(joinType, buildSide) =>
                    Seq(createBroadcastRangeJoinExec(
                      leftKeys, rightKeys, buildSide, rangeJoin))
                }
              case _ => None
            }
          }
