...
private[execution] sealed abstract class RangeIndexKind {
  def matches(rangeJoin: RangeJoin): Boolean
}

private[execution] case object IntervalIndexKind extends RangeIndexKind {
  override def matches(rangeJoin: RangeJoin): Boolean = {
    rangeJoin == PointInRangeJoin || rangeJoin == IntervalOverlapJoin
  }
}

private[execution] case object PointIndexKind extends RangeIndexKind {
  override def matches(rangeJoin: RangeJoin): Boolean = rangeJoin.isInstanceOf[PartialRangeJoin]
}
...
private[execution] case class RangeBroadcastMode private(
  ...
  /** Ordering of the value `RangeIndex.getValue` reads. A UDT uses its sql type. */
  private def keyOrdering(key: Expression) =
    PhysicalDataType.ordering(RangeIndex.indexedType(key.dataType))
  ...
  override def canonicalized: RangeBroadcastMode =
