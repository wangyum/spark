...
import org.apache.spark.sql.catalyst.planning.{
  IntervalOverlapJoin, PartialRangeJoin, PointInRangeJoin, RangeJoin}
...
private[execution] case class RangeBroadcastMode(
  ...
  override def canonicalized: RangeBroadcastMode = copy(
    buildKeys = buildKeys.map(_.canonicalized))

  override def toString: String = "RangeBroadcastMode"
}
