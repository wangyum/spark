 ...
 * emitted, because the broadcast is copied to every partition. A range join matches rows
 * whose join condition can be reduced to one of the following forms:
 *
 ...
 *    }}}
 * 2. Interval overlap: `(low, high)` on each side, probed with `overlapping`:
 *    {{{
 *      SELECT A.*, B.*
 *      FROM   tableA A JOIN tableB B
 *      ON     A.lo < B.hi AND B.lo < A.hi
 *    }}}
 * 3. Partial range: a single inequality between one column on each side, e.g.:
 *    {{{
 *      SELECT A.*, B.*
 *      FROM   tableA A JOIN tableB B
 *      ON     A.start < B.end
...
object BroadcastRangeJoinExec {
  ...
  /**
   * Candidates for one streamed row. A null key is unknown and matches nothing.
   * Called from generated code; the index still returns a superset.
   */
  def probeCandidates(
      join: BroadcastRangeJoinExec,
      relation: RangeRelation,
      keys: Array[AnyRef]): Iterator[InternalRow] = {
    if (keys == null || keys.exists(_ == null)) {
      Iterator.empty
    } else {
      join.candidates(relation, keys.toIndexedSeq)
    }
  }
}
...
case class BroadcastRangeJoinExec(
  ...
  override protected def stringArgs: Iterator[Any] =
    Iterator(joinType, buildSide, condition, rangeJoin)
  ...
  override def verboseStringWithOperatorId(): String = {
    s"""
       ...
       |${ExplainUtils.generateFieldString("Range join type", rangeJoin.toString)}
       |${ExplainUtils.generateFieldString(
          "Join condition", condition.map(_.toString).getOrElse("None"))}
       |""".stripMargin
  }
        ...
        private def findNextMatch(): Boolean = {
          while (streamRow != null || stream.hasNext) {
            if (streamRow == null) {
              ...
              matchIterator =
                if (keys.exists(_ == null)) Iterator.empty
                else candidates(index, keys)
            }
  ...
  override def supportCodegen: Boolean = {
    BroadcastRangeJoinExec.supports(joinType, buildSide) &&
      (condition.toSeq ++ streamedKeys).forall(expressionCodegens)
  }

  /** Non-leaf [[CodegenFallback]] needs an input row, which this consume path does not set. */
  private def expressionCodegens(expression: Expression): Boolean = !expression.exists {
  ...
  override def doConsume(ctx: CodegenContext, input: Seq[ExprCode], row: ExprCode): String = {
    joinType match {
      ...
      case LeftSemi => codegenExistence(ctx, input, exists = true)
      case LeftAnti => codegenExistence(ctx, input, exists = false)
      case t =>
        throw new IllegalArgumentException(
  ...
  private def genCandidateIterator(
    ...
    val joinTerm = ctx.addReferenceObj("rangeJoin", this)
    val code =
      s"""
         |$iterCls $matches = org.apache.spark.sql.execution.joins
         |  .BroadcastRangeJoinExec.probeCandidates(
         |    $joinTerm, $relationTerm, new Object[] {${keyVars.mkString(", ")}});
       """.stripMargin
    (matches, code)
  }
