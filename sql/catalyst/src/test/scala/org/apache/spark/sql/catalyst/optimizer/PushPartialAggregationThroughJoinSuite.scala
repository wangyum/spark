...
class PushPartialAggregationThroughJoinSuite
  ...
  private def sumWithDataType(
    ...
    Sum(sum, EvalMode.fromSQLConf(SQLConf.get), resultDataType = dataType).toAggregateExpression()
  }
    ...
    Seq(-1, 10000).foreach { threshold =>
      withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> threshold.toString) {
        Seq(Inner, LeftOuter, RightOuter, FullOuter).foreach { joinType =>
          ...
          val correctLeft =
            PartialAggregate(Seq('a, 'b, 'c), Seq('a, 'b, 'c), testRelation1).as("l")
          val correctRight =
            PartialAggregate(Seq('x), Seq('x), testRelation2.select('x)).as("r")
          ...
          correctLeft.join(correctRight,
            ...
            .groupBy('b)(Cast(Divide(CheckOverflowInSum(
              ...
              Cast(sumWithDataType($"l._pushed_count_c" * $"r.cnt", ansiEnabled,
                Some(LongType)), DecimalType(20, 0)), EvalMode.LEGACY),
