...
object SQLConf {
  ...
  val DEDUPLICATE_SEMI_ANTI_JOIN_ENABLED =
    buildConf("spark.sql.optimizer.deduplicateRightSideOfLeftSemiAntiJoin.enabled")
      .doc("When true, we will deduplicate the right side of left semi/anti join.")
      .version("3.4.0")
      .booleanConf
      .createWithDefault(true)

  val PUSH_PARTIAL_AGGREGATION_THROUGH_JOIN_ENABLED =
    buildConf("spark.sql.optimizer.pushPartialAggregationThroughJoin.enabled")
      .doc("When true, we will push partial aggregationThroughJoin.")
      .version("3.4.0")
      .booleanConf
      .createWithDefault(true)
