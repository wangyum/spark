...
object ConstantPropagation extends Rule[LogicalPlan] {
  ...
  private def replaceConstants(condition: Expression, equalityPredicates: EqualityPredicates)
    : Expression = {
    val defaultConstantsMap = AttributeMap(equalityPredicates.map(_._1))
    val defaultPredicates = equalityPredicates.map(_._2).toSet
    ...
    condition transform {
      case b: BinaryComparison =>
        if (!defaultPredicates.contains(b)) {
          replaceConstants0(b, defaultConstantsMap)
        } else {
          val excludedEqualityPredicates = equalityPredicates.filterNot(_._2.semanticEquals(b))
          val constantsMap = AttributeMap(excludedEqualityPredicates.map(_._1))
          replaceConstants0(b, constantsMap)
        }
    }
  }
}
